...
| github<br>workflows<br>review_card_check `boolean` |false| Enforce in the review workflow that every commit message ends with a card number reference like "#1234" or a Jira issue key like "NEB-123". Off by default; repos that track work in cards should opt in.|
